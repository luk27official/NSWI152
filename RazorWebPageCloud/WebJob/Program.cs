...
public class Program
{
    static async Task Main(string[] args)
    {
        ...
        var host = Host.CreateDefaultBuilder()
            ...
            {
                app.AddJsonFile("appsettings.json"); // for local development, otherwise set the env vars in Azure
...
public class Worker : IHostedService, IDisposable
{
    ...
    private async Task SendMailAsync(string recipients, string? subject, string? body, int mailId, CancellationToken stoppingToken)
    {
        ...
        smtpClient.Host = configuration.GetValue<string>("MailSettings_SmtpHost");
        // Console.WriteLine(smtpClient.Host);
        smtpClient.Credentials = new NetworkCredential(configuration.GetValue<string>("MailSettings_SmtpUsername"), configuration.GetValue<string>("MailSettings_SmtpPassword"));

        await smtpClient.SendMailAsync(
            from: configuration.GetValue<string>("MailSettings_SmtpFrom"),
